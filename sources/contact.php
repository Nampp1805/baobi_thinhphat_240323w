<?php 
	...
	if(!empty($_POST['submit-contact']))
	{
        ...
        if(($scoreCaptcha >= 0.5 && $actionCaptcha == 'contact') || $testCaptcha == true)
		{
			...
			if($emailer->send("admin", $arrayEmail, $subject, $message, $file))
			{
				...
				if($emailer->send("customer", $arrayEmail, $subject, $message, $file)) $func->transfer(guilienhethanhcong, $configBase);
			}
	...
	/* SEO */
	$seoDB = $seopage = $d->rawQueryOne("select * from #_seopage where type = ? limit 0,1",array('lien-he'));
