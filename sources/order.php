<?php
	...
	if($_SESSION[$loginMember]['id'])
	{
		$rowDetail = $d->rawQueryOne("select fullname, username, gender, birthday, email, phone, address from #_member where id = ? limit 0,1",array($_SESSION[$loginMember]['id']));
	} else {
		$rowDetail = '';
	}
	/* Tỉnh thành */
	$city = $d->rawQuery("select name$lang, id from #_city order by id asc");
	...
	if(!empty($_POST['thanhtoan']))
	{
		...
		{
			$func->transfer(donhangkhonghople, $configBase, false);
		}
		...
		if(!empty($dataOrder))
		{
		    ...
		    $city_text = $func->getInfoDetail('namevi', "city", $city);
		    $district_text = $func->getInfoDetail('namevi', "district", $district);
		    $ward_text = $func->getInfoDetail('namevi', "ward", $ward);
		    $address = htmlspecialchars($dataOrder['address']).', '.$ward_text['namevi'].', '.$district_text['namevi'].', '.$city_text['namevi'];
	    ...
	    /* Xóa giỏ hàng */
        unset($_SESSION['cart']);
		$func->transfer(thongtindonhangguithanhcong, $configBase);
	}
?>