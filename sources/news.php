<?php  
	...
	if($id!='')
	{
		...
		/* Lấy hình ảnh con */
		$rowDetailPhoto = $d->rawQuery("select photo, name$lang from #_gallery where id_parent = ? and com='news' and type = ? and kind='man' and val = ? and find_in_set('hienthi',status) order by numb,id desc",array($rowDetail['id'],$type,$type));
		...
		$curPage = $getPage;
		$perPage = (isset($optsetting['tt1']) && $optsetting['tt1']!='') ? htmlspecialchars($optsetting['tt1']) : 8;
