...
if ($id != '') {
	...
	/* Lấy hình ảnh con */
	$rowDetailPhoto = $d->rawQuery("select photo, name$lang from #_gallery where id_parent = ? and com='product' and type = ? and kind='man' and val = ? and find_in_set('hienthi',status) order by numb,id desc", array($rowDetail['id'], $type, $type));

	/* Sản phẩm đã xem*/
	//$func->product_seen_exists($rowDetail['id']);

	//$product_daxem = $d->rawQuery("select photo, name$lang, slugvi, slugen, sale_price, regular_price, discount, id,type from #_product where type = ? and find_in_set('hienthi',status) and find_in_set(id,'" . implode(',', $_SESSION['pro_seen']) . "') order by numb,id desc", array($type));
	...
	$curPage = $getPage;
	$perPage = (isset($optsetting['sp2']) && $optsetting['sp2'] != '') ? htmlspecialchars($optsetting['sp2']) : 8;
	...
	$curPage = $getPage;
	$perPage = (isset($optsetting['sp1']) && $optsetting['sp1'] != '') ? htmlspecialchars($optsetting['sp1']) : 8;
	$startpoint = ($curPage * $perPage) - $perPage;
	$limit = " limit " . $startpoint . "," . $perPage;
	$sql = "select photo, name$lang, slugvi, slugen, sale_price, regular_price, discount, id from #_product where $where order by numb,id desc $limit";
	$product = $d->rawQuery($sql, $params);
