<?php
	...
	function viewMans()
	{
		global $d, $func, $strUrl, $curPage, $items, $paging, $minTotal, $maxTotal, $price_from, $price_to, $allNewOrder, $totalNewOrder, $allConfirmOrder, $totalConfirmOrder, $allDeliveriedOrder, $totalDeliveriedOrder, $allCanceledOrder, $totalCanceledOrder, $totalSendOrder, $allSendOrder;
		...
		$order_count = $d->rawQueryOne("select count(id), sum(total_price) from #_order where order_status = 3");
		$allSendOrder = $order_count['count(id)'];
		$totalSendOrder = $order_count['sum(total_price)'];
