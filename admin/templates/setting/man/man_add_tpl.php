            ...
            <div class="card-body">
                        ...
                        <div class="tab-content" id="custom-tabs-three-tabContent-lang">
                            ...
                            <div class="tab-pane fade show <?= ($k == 'vi') ? 'active' : '' ?>" id="tabs-lang-<?= $k ?>"
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control text-sm" name="data[address<?= $k ?>]"
                                        id="address<?= $k ?>" placeholder="Địa chỉ (<?= $k ?>)"
                                        value="<?= (!empty($flash->has('address' . $k))) ? $flash->get('address' . $k) : @$item['address' . $k] ?>"
                                        required>
                                </div>
                                <div class="form-group">
                                    <label for="copyright<?= $k ?>">Tên copyright (<?= $k ?>):</label>
                                    <input type="text" class="form-control text-sm" name="data[copyright<?= $k ?>]"
                                        id="copyright<?= $k ?>" placeholder="Tên copyright (<?= $k ?>)"
                                        value="<?= (!empty($flash->has('copyright' . $k))) ? $flash->get('copyright' . $k) : @$item['copyright' . $k] ?>"
                                        required>
                                </div>
                            </div>
                            <?php } ?>
                ...
                <div class="row">
                    ...
                    <div class="form-group col-md-4 col-sm-6">
                        ...
                        <input type="worktime" class="form-control text-sm" name="data[options][worktime]" id="worktime"
                    ...
                    <div class="form-group col-md-4 col-sm-6">
                        ...
                        <input type="text" class="form-control text-sm" name="data[options][link_googlemaps]"
                            id="link_googlemaps" placeholder="link google maps"
                            value="<?= (!empty($flash->has('link_googlemaps'))) ? $flash->get('link_googlemaps') : @$options['link_googlemaps'] ?>">
                    </div>
                    <?php } ?>
                    <?php if (isset($config['setting']['buuchinh']) && $config['setting']['buuchinh'] == true) { ?>
                    <div class="form-group col-md-4 col-sm-6">
                        <label for="tinh">Tên tỉnh / Thành phố:</label>
                        <input type="text" class="form-control text-sm" name="data[options][tinh]" id="tinh"
                            placeholder="Tên tỉnh / Thành phố"
                            value="<?= (!empty($flash->has('tinh'))) ? $flash->get('tinh') : @$options['tinh'] ?>"
                            required>
                    </div>
                    <div class="form-group col-md-4 col-sm-6">
                        <label for="mavung">Mã bưu chính:</label>
                        <input type="text" class="form-control text-sm" name="data[options][mavung]" id="mavung"
                            placeholder="Mã bưu chính"
                            value="<?= (!empty($flash->has('mavung'))) ? $flash->get('mavung') : @$options['mavung'] ?>"
                            required>
                    </div>
                    <?php } ?>
                    <?php if (isset($config['setting']['trangchu']) && $config['setting']['trangchu'] == true) { ?>
                    <div class="form-group col-md-4 col-sm-6">
                        <label for="sp">Số sản phẩm hiển thị trang chủ:</label>
                        <input type="number" class="form-control text-sm" name="data[options][sp]" id="sp"
                            placeholder="Số sản phẩm hiển thị trang chủ"
                            value="<?= (!empty($flash->has('sp'))) ? $flash->get('sp') : @$options['sp'] ?>" required>
                    </div>
                    <?php } ?>
                    <?php if (isset($config['setting']['sanpham']) && $config['setting']['sanpham'] == true) { ?>
                    <div class="form-group col-md-4 col-sm-6">
                        <label for="sp1">Số sản phẩm / trang:</label>
                        <input type="number" class="form-control text-sm" name="data[options][sp1]" id="sp1"
                            placeholder="Số sản phẩm / trang"
                            value="<?= (!empty($flash->has('sp1'))) ? $flash->get('sp1') : @$options['sp1'] ?>"
                            required>
                    </div>
                    <div class="form-group col-md-4 col-sm-6">
                        <label for="sp2">Số sản phẩm liên quan / trang:</label>
                        <input type="number" class="form-control text-sm" name="data[options][sp2]" id="sp2"
                            placeholder="Số sản phẩm liên quan / trang"
                            value="<?= (!empty($flash->has('sp2'))) ? $flash->get('sp2') : @$options['sp2'] ?>"
                            required>
                    </div>
                    <?php } ?>
                    <?php if (isset($config['setting']['baiviet']) && $config['setting']['baiviet'] == true) { ?>
                    <div class="form-group col-md-4 col-sm-6">
                        <label for="tt">Bài viết / trang:</label>
                        <input type="number" class="form-control text-sm" name="data[options][tt]" id="tt"
                            placeholder="Bài viết / trang"
                            value="<?= (!empty($flash->has('tt'))) ? $flash->get('tt') : @$options['tt'] ?>" required>
                    </div>
                    <div class="form-group col-md-4 col-sm-6">
                        <label for="tt1">Bài viết liên quan / trang:</label>
                        <input type="number" class="form-control text-sm" name="data[options][tt1]" id="tt1"
                            placeholder="Bài viết liên quan / trang"
                            value="<?= (!empty($flash->has('tt1'))) ? $flash->get('tt1') : @$options['tt1'] ?>"
                            required>
                    </div>
                    <?php } ?>
                </div>
