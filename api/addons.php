...
<?php if($type == 'video-fotorama') {
	$video_home = $d->rawQuery("select link_video, id, name$lang from #_photo where type = ? and act <> ? and find_in_set('hienthi',status) order by numb, id desc",array('video','photo_static')); if(count($video_home)) { ?>
...
<?php if($type == 'script-main') { ?>
	<div id="fb-root"></div>
	<script async defer crossorigin="anonymous" src="https://connect.facebook.net/<?php if($lang=='en')echo 'en_EN';else echo 'vi_VN' ?>/sdk.js#xfbml=1&version=v7.0&autoLogAppEvents=1"></script>
